                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="live-preview">
                            <form id="countryForm" method="post" action="{{route('document_update', $data->id)}}" enctype="multipart/form-data">
                            @csrf
                            <div class="row gy-4">
...
@include('includes.admin.choice_language_update', ['languages' => $languages, 'country'=>$data])
  ...
  .onSuccess(async (event) => {
    // event.target.submit();
      ...
      try {
        ...
        const response = await axios.post('{{route('document_update', $data->id)}}', formData)
        ...
        if(error?.response?.data?.errors?.document){
            validation.showErrors({'#document': error?.response?.data?.errors?.document[0]})
        }
      } finally{
            submitBtn.innerHTML =  `
                Update
